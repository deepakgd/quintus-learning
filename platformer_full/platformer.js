...
Q.Sprite.extend("Player",{
  ...
  init: function(p) {
    ...
    this.on("bump.top","breakTile"); // warning box hittable item - ref: http://www.html5quintus.com/guide/2d.md#.XS7993Uzar4 for bump top event

    this.on("sensor.tile","checkLadder"); // sensor="true" from tiled map - allow us to run on tiles
    this.on("enemy.hit","enemyHit"); // trigger when player hit on enemy
    this.on("jump"); // player jump event - to play music while jump
    this.on("jumped"); // after jump - set false to avoid to manu calls check jump and jumped for more details

    Q.input.on("down",this,"checkDoor"); // down arrw check door and process if door
  },
  ...
  // set false after jump completion
  jumped: function(obj) {
    obj.p.playedJump = false;
  },

  // trigger while crossing ladder and also trigger if tile set to sensor=rue
  // allows us to go up and down in ladder
  ...
  // check current tile is door or not - step function will check and allow  us to go inside door
  checkDoor: function() {
  ...
  // play animation such as stand and walk while player on ladder 
  ...
  // this is like update invoke continously
  step: function(dt) {
    var processed = false;
